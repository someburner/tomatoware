...
############ ################################################################
do_LIBICONV() {
LIBICONV_VERSION=${VMAP[libiconv]}
...
############### #############################################################
do_LIBREADLINE() {
LIBREADLINE_VERSION=${VMAP[libreadline]}
...
############# ###############################################################
do_LIBSIGCpp() {
LIBSIGCPLUSPLUS_VERSION=${VMAP[libsigc++]}
...
############ ################################################################
do_LIBEVENT() {
LIBEVENT_VERSION=${VMAP[libevent]}
...
################## ##########################################################
do_LIBMYSQLCLIENT() {
LIBMYSQLCLIENT_VERSION=${VMAP[libmysqlclient]}
...
######## ####################################################################
do_PERL() {
PERL_VERSION=${VMAP[perl]}
PERL_CROSS_VERSION=${VMAP[perlcross]}
...
if ! [[ -f .patched ]]; then
	for file in $PATCHES/python/*.patch; do
		patch -p1 < "$file"
	done
	autoreconf
...
############### #############################################################
do_PAR2CMDLINE() {
PAR2CMDLINE_VERSION=${VMAP[par2cmdline]}
...
if ! [[ -f .patched ]]; then
	for file in $PATCHES/python3/*.patch; do
		patch -p1 < "$file"
	done
	autoreconf
...
if ! [[ -f .installed ]]; then
	PATH=$SRC/python3/native3/bin:$PATH \
	make install DESTDIR=$BASE --keep-going || true
	touch .installed
fi
}
...
################## ##########################################################
do_RAPIDJSON() {
RAPIDJSON_VERSION=${VMAP[rapidjson]}
